...
preg の a{n,m} 系 と基本同じ  最長一致

```
a\{n}    丁度 n 回の a の繰り返しにマッチ
a\{n,}   n回以上の a の繰り返しにマッチ           最長マッチ
a\{n,m}  n回以上 m 回以下の a の繰り返しにマッチ  最長マッチ
a\{,m}   0 回以上 m 回以下の a の繰り返しにマッチ 最長マッチ
a\{,}    0 回以上 a の繰り返しにマッチ            最長マッチ, a* と同じ
```


## a\{-n,m} 系

a\{n,m} と基本同じだがブレス内の頭に - をつけることで最短一致になる

```
a\{-n}    n 回の a の繰り返しにマッチ              -のアリナシで意味に違いはない
a\{-n,}   n 回以上の a の繰り返しにマッチ          最短マッチ preg の a{n,}?  と同じ
a\{-n,m}  n 回以上 m 回以下の a の繰り返しにマッチ 最短マッチ preg の a{n,m}? と同じ
a\{-,m}   0 回以上 m 回以下の a の繰り返しにマッチ 最短マッチ preg の a{,m}?  と同じ
a\{-,}    0 回以上 a の繰り返しにマッチ            最短マッチ preg の a*?     と同じ
a\{-,1}   0 か 1 個 の a にマッチ                  最短マッチ preg の a??     と同じ
```
...
```
\%V       は ビジュアルモードの範囲内にマッチ
\%Vfoo\%V は 選択範囲内の foo にマッチ
